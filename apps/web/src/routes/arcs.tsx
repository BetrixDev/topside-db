...
import type { ArcSearchHit } from "@topside-db/schemas";
...
export const Route = createFileRoute("/arcs")({
  component: ArcsPage,
  validateSearch: zodValidator(arcsSearchSchema),
...
function ArcsPage() {
  ...
  const arcs = (data?.hits ?? []) as ArcSearchHit[];
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <header className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 rounded-2xl bg-linear-to-br from-red-500/20 to-red-500/5 border border-red-500/20 flex items-center justify-center">
              <ZapIcon className="w-6 h-6 text-red-500" />
            </div>
            <div>
              <h1 className="text-3xl font-bold tracking-tight">Arcs</h1>
              <p className="text-muted-foreground text-sm">
                Get to know your common enemies
              </p>
            </div>
          </div>
            ...
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              <SearchIcon
                className={cn(
                  ...
                  inputValue ? "text-red-500" : "text-muted-foreground"
                )}
              />
            </div>
            <Input
          ...
          {isLoading ? (
            <div className="space-y-3">
              {Array.from({ length: 8 }).map((_, i) => (
                <ArcCardSkeleton key={i} delay={i * 50} />
              ))}
            </div>
          ) : arcs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-muted-foreground max-w-md">
                We couldn't find any Arcs matching "{q}". Try adjusting your
                search terms.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="space-y-2">
                {arcs.map((arc, index) => (
                  <ArcCard key={arc.id} arc={arc} index={index} />
                ))}
              </div>
            </>
          )}
...
function ArcCard({ arc, index }: { arc: ArcSearchHit; index: number }) {
  return (
    <Link
      to="/arcs/$arcId"
      params={{ arcId: arc.id }}
    ...
    >
      <div
        className={cn(
          ...
          "hover:bg-card/90 hover:border-red-500/30 hover:shadow-lg hover:shadow-red-500/5",
          "transition-all duration-300 ease-out",
          "animate-in fade-in slide-in-from-bottom-2"
        )}
      >
        {/* Arc Image */}
        <div className="relative w-16 h-16 rounded-xl bg-linear-to-br from-background to-muted/50 border border-border/30 flex items-center justify-center overflow-hidden shrink-0 group-hover:border-red-500/30 transition-colors">
          {arc.imageUrl ? (
            <img
              src={arc.imageUrl}
              alt={arc.name}
            ...
            <ZapIcon className="w-7 h-7 text-muted-foreground/50" />
          )}
        </div>

        {/* Arc Info */}
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-foreground group-hover:text-red-500 transition-colors truncate">
            {arc.name}
          </h3>
            ...
            {arc.threatLevel && (
              <span
                className={cn(
                  ...
                  getThreatLevelColor(arc.threatLevel)
                )}
              >
...
function ArcCardSkeleton({ delay = 0 }: { delay?: number }) {
  return (
    <div
